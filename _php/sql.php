...
class SQL {
  ...
  'get_project_works' => 'select pp.ID, p.NAME, p.COST ,pp.REAL_UNIT as UNIT, p.COST*pp.REAL_UNIT as TOTAL from project_price pp left join price p on pp.PRICE_ID = p.ID where pp.PROJECT_ID = [@ID]',
  ...
  'getComments' => "select * from messages where project_id = [@ID]",
  'addNewComments' => "insert into messages (project_id, message) values([@ID],'[@MESSAGE]')",
  'get_work_pipls' => "select ID, ONAME as NAME from resources where fname = 2"  
  ];
